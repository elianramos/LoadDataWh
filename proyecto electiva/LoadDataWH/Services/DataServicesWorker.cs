...
{ 
        ...
        private async Task<OperactionResult> LoadFactClientesAtendidos()
        {
            ...
            try
            {
                ...
                var customServed = await dbContexNorthwind.ViewsClientesActendidos
                    .AsNoTracking()
                    .ToListAsync();
        ...
        private async Task<OperactionResult> LoadFactVistaVentas()
            {
                ...
                try
                {
                    var order = await dbContexNorthwind.ViewsVistasVentas.AsNoTracking().ToListAsync();
                    ...
                    foreach (var item in order)
                    {
                        ...
                        var date = await dbContextDwhNorthwind.DimDate.FirstOrDefaultAsync(e => e.Fecha == item.OrderDate);
            ...
            private async Task<OperactionResult> LoadDimDate()
            {
                ...
                try
                {
                   ...
                   .ToListAsync();
                    ...
                    if (date.Any())
                    {
                        await dbContextDwhNorthwind.DimDate.Where(d => date.Contains(d.ID)).AsNoTracking().ExecuteDeleteAsync();
                    }
            ...
            private async Task<OperactionResult> LoadDimCustomers()
            {
                ...
                try
                {
                    ...
                    await dbContextDwhNorthwind.Database.ExecuteSqlRawAsync("TRUNCATE TABLE [DimCustomers]");
